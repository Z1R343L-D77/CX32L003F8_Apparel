 ...
 *  @note   数据传输低位在前，高位在后
 */
 BOOLEAN FD612_WrByte( INT8U dat )
{
	INT8U i;				        /* 移位写出的位数寄存器 */
  BOOLEAN ACKTemp=0;
	FD612_SDA_D_OUT;		        /* 设置SDA为输出方向 */
...
void FD612_Init( void)//<共阳参考
{
  INT8U _i;        INT8U i;
	    ...
	    INT8U buf[12] = {0};  // 地址00H~0BH，共12字节全清零
			
  FD612_8SEG_MODE;
  for(_i=0;_i<12;_i++){
    FD612_DispBuff.DispData[_i]= 0X00;
  }
  FD612_DispStateWr(FD612_INTENS8|FD612_DISP_ON);  
  FD612_Refresh();
...
//<读取按键
unsigned int FD612_RdKey()
{
    INT32U KeyTemp=0;
    FD612_Start();
    FD612_WrByte(FD612_KEY_RDCMD);
    FD612_DELAY_WAIT;
    FD612_SDA_SET;                  /* 设置SDA为高电平，释放总线 */
    FD612_SDA_D_IN;                 /* 设置SDA为输入方向 */
    FD612_RdByte(FD612_ACK);
...
void Full_Scan(void)
{
    ...
    // 88.8
		// 88.8
		//以下两个dp是固定
		//第一个数码管 0x20 是第一个，40第二个 80同理
		buf[11] = 0x80;//d
		buf[10] = 0x80;//c
		buf[9] = 0x80;//b
		buf[8] = 0x80;//a
		//02是第一个数码管，04第二个，08第三个，
		buf[3] = 0x08;//e
		buf[2] = 0x08;//f 
		buf[1] = 0x04;//dp1 仅中间数码管
		buf[0] = 0x08;//g

		//第二个数码管 0x10就是最右边第一个数码管，0x20就是第二个，0x30就是第二个和第一个，同理0x40
		buf[7] = 0x10;//c
		buf[6] = 0x10;//d
		buf[5] = 0x10;//b
    buf[4] = 0x10;//a
